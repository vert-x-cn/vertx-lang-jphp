...
public class PebbleTemplateEngine extends BaseWrapper<PebbleTemplateEngine>{
  ...
  this.render = function() {
    ...
    if (__args.length === 3 && typeof __args[0] === 'object' && __args[0]._jdel && typeof __args[1] === 'string' && typeof __args[2] === 'function') {
      j_pebbleTemplateEngine.render(__args[0]._jdel, __args[1], function(ar) {
      if (ar.succeeded()) {
      ...
      j_pebbleTemplateEngine.render(__args[0]._jdel, __args[1], __args[2], function(ar) {
      if (ar.succeeded()) {
  ...
  this.isCachingEnabled = function() {
    var __args = arguments;
    if (__args.length === 0) {
      return j_pebbleTemplateEngine.isCachingEnabled();
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.setExtension = function(extension) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'string') {
      j_pebbleTemplateEngine.setExtension(extension);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.setMaxCacheSize = function(maxCacheSize) {
    ...
    if (__args.length === 1 && typeof __args[0] ==='number') {
      j_pebbleTemplateEngine.setMaxCacheSize(maxCacheSize);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
...
PebbleTemplateEngine.create = function(vertx) {
  ...
  if (__args.length === 1 && typeof __args[0] === 'object' && __args[0]._jdel) {
    return utils.convReturnVertxGen(PebbleTemplateEngine, JPebbleTemplateEngine.create(vertx._jdel));
  } else throw new TypeError('function invoked with invalid arguments');
};

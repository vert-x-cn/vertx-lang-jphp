...
public class HandlebarsTemplateEngine extends BaseWrapper<HandlebarsTemplateEngine>{
  ...
  this.render = function() {
    ...
    if (__args.length === 3 && typeof __args[0] === 'object' && __args[0]._jdel && typeof __args[1] === 'string' && typeof __args[2] === 'function') {
      j_handlebarsTemplateEngine.render(__args[0]._jdel, __args[1], function(ar) {
      if (ar.succeeded()) {
      ...
      j_handlebarsTemplateEngine.render(__args[0]._jdel, __args[1], __args[2], function(ar) {
      if (ar.succeeded()) {
  ...
  this.isCachingEnabled = function() {
    var __args = arguments;
    if (__args.length === 0) {
      return j_handlebarsTemplateEngine.isCachingEnabled();
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.setExtension = function(extension) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'string') {
      j_handlebarsTemplateEngine.setExtension(extension);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.setMaxCacheSize = function(maxCacheSize) {
    ...
    if (__args.length === 1 && typeof __args[0] ==='number') {
      j_handlebarsTemplateEngine.setMaxCacheSize(maxCacheSize);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
...
HandlebarsTemplateEngine._jclass = utils.getJavaClass("io.vertx.ext.web.templ.HandlebarsTemplateEngine");

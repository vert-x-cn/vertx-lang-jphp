...
public class Route extends BaseWrapper<Route>{
  ...
  this.method = function(method) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'string') {
      j_route.method(io.vertx.core.http.HttpMethod.valueOf(method));
      return that;
  ...
  this.blockingHandler = function() {
    var __args = arguments;
    if (__args.length === 1 && typeof __args[0] === 'function') {
      j_route.blockingHandler(function(jVal) {
  ...
  this.setRegexGroupsNames = function(groups) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'object' && __args[0] instanceof Array) {
      j_route.setRegexGroupsNames(utils.convParamListBasicOther(groups));
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };

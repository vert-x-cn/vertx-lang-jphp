...
public class User extends BaseWrapper<User>{
  ...
  this.isAuthorized = function(authority, resultHandler) {
    ...
    if (__args.length === 2 && typeof __args[0] === 'string' && typeof __args[1] === 'function') {
      j_user.isAuthorized(authority, function(ar) {
      if (ar.succeeded()) {
  ...
  this.isAuthorised = function(authority, resultHandler) {
    var __args = arguments;
    if (__args.length === 2 && typeof __args[0] === 'string' && typeof __args[1] === 'function') {
      j_user.isAuthorised(authority, function(ar) {
      if (ar.succeeded()) {
        resultHandler(ar.result(), null);
      } else {
        resultHandler(null, ar.cause());
  ...
  this.setAuthProvider = function(authProvider) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'object' && __args[0]._jdel) {
      j_user.setAuthProvider(authProvider._jdel);
    } else throw new TypeError('function invoked with invalid arguments');
  };

...
public class Pump extends BaseWrapper<Pump>{
  ...
  this.setWriteQueueMaxSize = function(maxSize) {
    ...
    if (__args.length === 1 && typeof __args[0] ==='number') {
      j_pump.setWriteQueueMaxSize(maxSize);
      return that;
...
Pump.pump = function() {
  ...
  if (__args.length === 2 && typeof __args[0] === 'object' && __args[0]._jdel && typeof __args[1] === 'object' && __args[1]._jdel) {
    return utils.convReturnVertxGen(Pump, JPump.pump(__args[0]._jdel, __args[1]._jdel));
    ...
    return utils.convReturnVertxGen(Pump, JPump.pump(__args[0]._jdel, __args[1]._jdel, __args[2]));
  } else throw new TypeError('function invoked with invalid arguments');
};

...
public class WriteStream extends BaseWrapper<WriteStream>{
  ...
  this.exceptionHandler = function(handler) {
    ...
    if (__args.length === 1 && (typeof __args[0] === 'function' || __args[0] == null)) {
      j_writeStream.exceptionHandler(handler == null ? null : function(jVal) {
  ...
  this.write = function(data) {
    ...
    if (__args.length === 1 && j_T.accept(__args[0])) {
      j_writeStream.write(j_T.unwrap(data));
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.end = function() {
    ...
    if (__args.length === 0) {
      ...
      j_writeStream.end(j_T.unwrap(__args[0]));
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.setWriteQueueMaxSize = function(maxSize) {
    ...
    if (__args.length === 1 && typeof __args[0] ==='number') {
      j_writeStream.setWriteQueueMaxSize(maxSize);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.drainHandler = function(handler) {
    var __args = arguments;
    if (__args.length === 1 && (typeof __args[0] === 'function' || __args[0] == null)) {
      j_writeStream.drainHandler(handler);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };

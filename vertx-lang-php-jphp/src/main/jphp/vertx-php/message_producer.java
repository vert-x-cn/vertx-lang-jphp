...
public class MessageProducer extends BaseWrapper<MessageProducer>{
  ...
  this.end = function() {
    ...
    if (__args.length === 0) {
      ...
      j_messageProducer.end(j_T.unwrap(__args[0]));
  ...
  this.send = function() {
    ...
    if (__args.length === 1 && j_T.accept(__args[0])) {
      return utils.convReturnVertxGen(MessageProducer, j_messageProducer.send(j_T.unwrap(__args[0])), undefined);
    }  else if (__args.length === 2 && j_T.accept(__args[0]) && typeof __args[1] === 'function') {
      return utils.convReturnVertxGen(MessageProducer, j_messageProducer.send(j_T.unwrap(__args[0]), function(ar) {
      if (ar.succeeded()) {
  ...
  this.exceptionHandler = function(handler) {
    ...
    if (__args.length === 1 && (typeof __args[0] === 'function' || __args[0] == null)) {
      j_messageProducer.exceptionHandler(handler == null ? null : function(jVal) {
  ...
  this.write = function(data) {
    var __args = arguments;
    if (__args.length === 1 && j_T.accept(__args[0])) {
      j_messageProducer.write(j_T.unwrap(data));
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.setWriteQueueMaxSize = function(maxSize) {
    ...
    if (__args.length === 1 && typeof __args[0] ==='number') {
      j_messageProducer.setWriteQueueMaxSize(maxSize);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.drainHandler = function(handler) {
    var __args = arguments;
    if (__args.length === 1 && (typeof __args[0] === 'function' || __args[0] == null)) {
      j_messageProducer.drainHandler(handler);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.deliveryOptions = function(options) {
    ...
    if (__args.length === 1 && (typeof __args[0] === 'object' && __args[0] != null)) {
      j_messageProducer.deliveryOptions(options != null ? new DeliveryOptions(new JsonObject(Java.asJSONCompatible(options))) : null);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };

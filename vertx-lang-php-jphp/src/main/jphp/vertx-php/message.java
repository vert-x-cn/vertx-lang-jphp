...
public class Message extends BaseWrapper<Message>{
  ...
  this.reply = function() {
    ...
    if (__args.length === 1 && typeof __args[0] !== 'function') {
      j_message.reply(utils.convParamTypeUnknown(__args[0]));
    }  else if (__args.length === 2 && typeof __args[0] !== 'function' && typeof __args[1] === 'function') {
      j_message.reply(utils.convParamTypeUnknown(__args[0]), function(ar) {
      if (ar.succeeded()) {
      ...
      j_message.reply(utils.convParamTypeUnknown(__args[0]), __args[1] != null ? new DeliveryOptions(new JsonObject(Java.asJSONCompatible(__args[1]))) : null);
    }  else if (__args.length === 3 && typeof __args[0] !== 'function' && (typeof __args[1] === 'object' && __args[1] != null) && typeof __args[2] === 'function') {
      j_message.reply(utils.convParamTypeUnknown(__args[0]), __args[1] != null ? new DeliveryOptions(new JsonObject(Java.asJSONCompatible(__args[1]))) : null, function(ar) {
      if (ar.succeeded()) {
  ...
  this.fail = function(failureCode, message) {
    ...
    if (__args.length === 2 && typeof __args[0] ==='number' && typeof __args[1] === 'string') {
      j_message.fail(failureCode, message);
    } else throw new TypeError('function invoked with invalid arguments');
  };

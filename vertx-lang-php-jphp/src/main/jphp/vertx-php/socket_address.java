...
public class SocketAddress extends BaseWrapper<SocketAddress>{
...
SocketAddress.inetSocketAddress = function(port, host) {
  ...
  if (__args.length === 2 && typeof __args[0] ==='number' && typeof __args[1] === 'string') {
    return utils.convReturnVertxGen(SocketAddress, JSocketAddress.inetSocketAddress(port, host));
  } else throw new TypeError('function invoked with invalid arguments');
};
...
SocketAddress.domainSocketAddress = function(path) {
  ...
  if (__args.length === 1 && typeof __args[0] === 'string') {
    return utils.convReturnVertxGen(SocketAddress, JSocketAddress.domainSocketAddress(path));
  } else throw new TypeError('function invoked with invalid arguments');
};

...
public class Pty extends BaseWrapper<Pty>{
  ...
  this.stdoutHandler = function(handler) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'function') {
      j_pty.stdoutHandler(function(jVal) {
      handler(jVal);
    });
      return that;
...
Pty.create = function() {
  ...
  if (__args.length === 0) {
    ...
    return utils.convReturnVertxGen(Pty, JPty.create(__args[0]));
  } else throw new TypeError('function invoked with invalid arguments');
};

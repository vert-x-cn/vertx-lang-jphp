...
public class Term extends BaseWrapper<Term>{
  Tty.call(this, j_val);
  ...
  this.stdinHandler = function(handler) {
    var __args = arguments;
    if (__args.length === 1 && typeof __args[0] === 'function') {
      j_term.stdinHandler(function(jVal) {
      handler(jVal);
    });
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  ...
  this.setSession = function(session) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'object' && __args[0]._jdel) {
      return utils.convReturnVertxGen(Term, j_term.setSession(session._jdel));
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.interruptHandler = function(handler) {
    var __args = arguments;
    if (__args.length === 1 && typeof __args[0] === 'object' && __args[0]._jdel) {
      j_term.interruptHandler(handler._jdel);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.suspendHandler = function(handler) {
    var __args = arguments;
    if (__args.length === 1 && typeof __args[0] === 'object' && __args[0]._jdel) {
      j_term.suspendHandler(handler._jdel);
      return that;
    } else throw new TypeError('function invoked with invalid arguments');
  };
  ...
  this.readline = function() {
    ...
    if (__args.length === 2 && typeof __args[0] === 'string' && typeof __args[1] === 'function') {
      j_term.readline(__args[0], function(jVal) {
      __args[1](jVal);
    });

...
public interface RedisClient<S extends io.vertx.redis.RedisClient> extends IWrapper<S>{
    ...
    @Signature
    default Memory evalScript(Environment __env__, Memory... args) {
    ...
    default Memory transaction(Environment __env__, Memory... args) {
        return Utils.convReturnVertxGen(__env__, RedisTransactionImpl::new, this.getWrappedObject().transaction()
    }
    ...
    @Signature
    default Memory clientReply(Environment __env__, Memory... args) {
        this.getWrappedObject().clientReply(options, handler)
    }

    @Signature
    default Memory hstrlen(Environment __env__, Memory... args) {
        this.getWrappedObject().hstrlen(key, field, handler)
    }

    @Signature
    default Memory touch(Environment __env__, Memory... args) {
        this.getWrappedObject().touch(key, handler)
    }

    @Signature
    default Memory touchMany(Environment __env__, Memory... args) {
        this.getWrappedObject().touchMany(keys, handler)
    }

    @Signature
    default Memory scriptDebug(Environment __env__, Memory... args) {
        this.getWrappedObject().scriptDebug(scriptDebugOptions, handler)
    }

    @Signature
    default Memory bitfield(Environment __env__, Memory... args) {
        this.getWrappedObject().bitfield(key, bitFieldOptions, handler)
    }

    @Signature
    default Memory bitfieldWithOverflow(Environment __env__, Memory... args) {

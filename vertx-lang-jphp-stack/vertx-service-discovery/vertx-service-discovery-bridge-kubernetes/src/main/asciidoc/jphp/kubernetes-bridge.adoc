...
Kubernetes services are mapped to `link:../../vertx-service-discovery/vertx-service-discovery-jphp/enums.adoc#Record[Record]`. This bridge only
supports the importation of services from kubernetes in vert.x (and not the opposite).

`link:../../vertx-service-discovery/vertx-service-discovery-jphp/enums.adoc#Record[Record]`are created from Kubernetes Service. The service type is deduced from the `service-type` label or from the port exposed by the service.

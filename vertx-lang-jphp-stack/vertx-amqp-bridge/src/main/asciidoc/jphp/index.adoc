...
Here is a simple example of creating a `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.MessageProducer.html[MessageProducer]` and sending a message with it.
First, an `link:https://vertx.okou.tk/phpdoc/vertx-amqp-bridge-jphp/classes/io.vertx.jphp.amqpbridge.AmqpBridge.html[AmqpBridge]` is created and started to establish the underlying AMQP connection,
then when this is complete the producer is created and a message sent using it. You can optionally supply a username
and password when starting the bridge, as well as supplying `link:../vertx-amqp-bridge-jphp/enums.adoc#AmqpBridgeOptions[AmqpBridgeOptions]` in order
...
Here is a simple example of creating a `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.MessageConsumer.html[MessageConsumer]` and registering a handler with it.
First, an `link:https://vertx.okou.tk/phpdoc/vertx-amqp-bridge-jphp/classes/io.vertx.jphp.amqpbridge.AmqpBridge.html[AmqpBridge]` is created and started to establish the underlying AMQP connection,
...
You can also optionally supply `link:../vertx-amqp-bridge-jphp/enums.adoc#AmqpBridgeOptions[AmqpBridgeOptions]` when creating the bridge in order to
...
recipients can be told where to send any responses required. The vert.x `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.Message.html[Message]`

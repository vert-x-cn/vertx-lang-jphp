...
topic, avoiding to register the handler for getting the messages and then using the `link:https://vertx.okou.tk/phpdoc/vertx-kafka-client-jphp/classes/io.vertx.jphp.kafka.client.consumer.KafkaConsumer.html#method_poll[poll]` method.
    ...
    $consumer->subscribe("test", function ($ar, $ar_err) {
        ...
        if ($ar != null) {
            ...
            $vertx->setPeriodic(1000, function ($timerId) {
                ...
                $consumer->poll(100, function ($ar1, $ar1_err) {

                    if ($ar1 != null) {

                        $records = $ar1;
                        for ($i = 0; $i < $records->size(); $i++) {
                            $record = $records->recordAt($i);
                            echo "key=".$record->key().",value=".$record->value().",partition=".$record->partition().",offset=".$record->offset()."\n";
                        };
                    };
                });

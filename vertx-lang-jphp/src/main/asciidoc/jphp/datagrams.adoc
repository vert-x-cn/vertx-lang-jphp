...
[source,jphp]
----
/*variableDecl*/$socket = $vertx.$createDatagramSocket(DatagramSocketOptions())
...
----
/*variableDecl*/$socket = $vertx.$createDatagramSocket(DatagramSocketOptions())
/*variableDecl*/$buffer = Buffer.$buffer("content")
// Send a Buffer
$socket.$send($buffer, 1234, "10.0.0.1", { asyncResult ->
  echo "Send succeeded? ${asyncResult.succeeded()}"."\n";
})
// Send a String
...
$socket.$listen(1234, "0.0.0.0", { asyncResult ->
  if (asyncResult.succeeded()) {
    ...
    echo "Listen failed${asyncResult.cause()}"."\n";
  }
})
...
$socket.$listen(1234, "0.0.0.0", { asyncResult ->
  if (asyncResult.succeeded()) {
    ...
    $socket.$listenMulticastGroup("230.0.0.1", { asyncResult2 ->
      echo "Listen succeeded? ${asyncResult2.succeeded()}"."\n";
    })
  } else {
    echo "Listen failed${asyncResult.cause()}"."\n";
  }
    ...
    $socket.$listenMulticastGroup("230.0.0.1", { asyncResult2 ->
      if (asyncResult2.succeeded()) {
        ...
        $socket.$unlistenMulticastGroup("230.0.0.1", { asyncResult3 ->
          echo "Unlisten succeeded? ${asyncResult3.succeeded()}"."\n";
        })
      } else {
        echo "Listen failed${asyncResult2.cause()}"."\n";
      }
    })
  } else {
    echo "Listen failed${asyncResult.cause()}"."\n";
...
$socket.$blockMulticastGroup("230.0.0.1", "10.0.0.2", { asyncResult ->
  echo "block succeeded? ${asyncResult.succeeded()}"."\n";
})

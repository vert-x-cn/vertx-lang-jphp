...
Be aware that the List will contain the `link:../../scaladocs/io/vertx/scala/core/dns/MxRecord.html[MxRecord]` sorted by the priority of them, which
means MX records with smaller priority coming first in the List.

The `link:../../scaladocs/io/vertx/scala/core/dns/MxRecord.html[MxRecord]` allows you to access the priority and the name of the MX record by offer methods for it like:
...
The `link:../../scaladocs/io/vertx/scala/core/dns/SrvRecord.html[SrvRecord]` allows you to access all informations contained in the SRV record itself:

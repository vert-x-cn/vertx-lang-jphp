...
[source,jphp]
----
/*variableDecl*/$client = $vertx.$createDnsClient(53, "10.0.0.1")
...
[source,jphp]
----
/*variableDecl*/$client = $vertx.$createDnsClient(DnsClientOptions(
  port = 53,
  host = "10.0.0.1",
  queryTimeout = 10000))
...
[source,jphp]
----
/*variableDecl*/$client1 = $vertx.$createDnsClient()

// Just the same but with a different query timeout
/*variableDecl*/$client2 = $vertx.$createDnsClient(DnsClientOptions(
  queryTimeout = 10000))
...
[source,jphp]
----
/*variableDecl*/$client = $vertx.$createDnsClient(53, "9.9.9.9")
$client.$lookup("vertx.io", { ar ->
  if (ar.succeeded()) {
    echo ar.result()."\n";
  } else {
    echo "Failed to resolve entry${ar.cause()}"."\n";
  }
})
...
$client.$resolveA("vertx.io", { ar ->
  if (ar.succeeded()) {
    /*variableDecl*/$records = ar.result()
    /*enhancedForLoop*/foreach ($records as $record) { 
      echo $record."\n";
    }
  } else {
    echo "Failed to resolve entry${ar.cause()}"."\n";

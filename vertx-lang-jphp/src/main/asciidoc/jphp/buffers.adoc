...
Buffers can create by using one of the static `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.buffer.Buffer.html#method_buffer[Buffer::buffer]` methods.
...
<?php
    use io\vertx\jphp\core\buffer\Buffer;
    $buff = Buffer::buffer("some string", "UTF-16");
...
<?php
    ...
    $i = 0;
    while ($i < $buff->length()) {
        echo "int value at ".$i." is ".$buff->getInt($i)."\n";
...
<?php
    use io\vertx\jphp\core\buffer\Buffer;
    $buff = Buffer::buffer(128);
    $pos = 15;
    $buff->setUnsignedByte($pos, 200);
    echo $buff->getUnsignedByte($pos)."\n";
...
Use `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.buffer.Buffer.html#method_length[length]` to obtain the length of the buffer.
The length of a buffer is the index of the byte in the buffer with the largest index + 1.

=== Copying buffers

Use `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.buffer.Buffer.html#method_copy[copy]` to make a copy of the buffer

=== Slicing buffers

A sliced buffer is a new buffer which backs onto the original buffer, i.e. it does not copy the underlying data.
Use `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.buffer.Buffer.html#method_slice[slice]` to create a sliced buffers

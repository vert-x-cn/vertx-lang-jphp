...
Buffers can create by using one of the static `link:../../scaladocs/io/vertx/scala/core/buffer/Buffer.html#buffer()[Buffer.buffer]` methods.
...
var i = 0
while(i < buff.length()){
  println(s"int value at ${i} is ${buff.getInt(i)}")
...
Use `link:../../scaladocs/io/vertx/scala/core/buffer/Buffer.html#length()[length]` to obtain the length of the buffer.
The length of a buffer is the index of the byte in the buffer with the largest index + 1.

=== Copying buffers

Use `link:../../scaladocs/io/vertx/scala/core/buffer/Buffer.html#copy()[copy]` to make a copy of the buffer

=== Slicing buffers

A sliced buffer is a new buffer which backs onto the original buffer, i.e. it does not copy the underlying data.
Use `link:../../scaladocs/io/vertx/scala/core/buffer/Buffer.html#slice()[slice]` to create a sliced buffers

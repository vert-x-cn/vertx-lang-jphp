...
The `link:../../scaladocs/io/vertx/scala/core/eventbus/EventBus.html[event bus]` is the *nervous system* of Vert.x.

There is a single event bus instance for every Vert.x instance and it is obtained using the method `link:../../scaladocs/io/vertx/scala/core/Vertx.html#eventBus()[eventBus]`.
...
Out of the box Vert.x allows any primitive/simple type, String, or `link:../../scaladocs/io/vertx/scala/core/buffer/Buffer.html[buffers]` to
be sent as messages.
...
This simplest way to register a handler is using `link:../../scaladocs/io/vertx/scala/core/eventbus/EventBus.html#consumer(java.lang.String,%20io.vertx.core.Handler)[consumer]`.
Here's an example:
...
eb.consumer("news.uk.sport", (message: io.vertx.scala.core.eventbus.Message<java.lang.Object>) => {
  println(s"I have received a message: ${message.body()}")
})

----

When a message arrives for your handler, your handler will be called, passing in the `link:../../scaladocs/io/vertx/scala/core/eventbus/Message.html[message]`.

The object returned from call to +consumer()+ is an instance of `link:../../scaladocs/io/vertx/scala/core/eventbus/MessageConsumer.html[MessageConsumer]`.

This object can subsequently be used to unregister the handler, or use the handler as a stream.

Alternatively you can use `link:../../scaladocs/io/vertx/scala/core/eventbus/EventBus.html#consumer(java.lang.String,%20io.vertx.core.Handler)[consumer]` to
...
consumer.handler((message: io.vertx.scala.core.eventbus.Message<java.lang.String>) => {
  println(s"I have received a message: ${message.body()}")
})
...
If you want to be notified when this has completed, you can register a `link:../../scaladocs/io/vertx/scala/core/eventbus/MessageConsumer.html#completionHandler(io.vertx.core.Handler)[completion handler]`
on the +MessageConsumer+ object.

[source,scala]
----
consumer.completionHandlerFuture().onComplete{
  case Success(result) => {
...
To unregister a handler, call `link:../../scaladocs/io/vertx/scala/core/eventbus/MessageConsumer.html#unregister()[unregister]`.

If you are on a clustered event bus, un-registering can take some time to propagate across the nodes.  If you want to
be notified when this is complete, use `link:../../scaladocs/io/vertx/scala/core/eventbus/MessageConsumer.html#unregister(io.vertx.core.Handler)[unregister]`.

[source,scala]
----
consumer.unregisterFuture().onComplete{
  case Success(result) => {
...
Publishing a message is simple. Just use `link:../../scaladocs/io/vertx/scala/core/eventbus/EventBus.html#publish(java.lang.String,%20java.lang.Object)[publish]` specifying the
...
You can send a message with `link:../../scaladocs/io/vertx/scala/core/eventbus/EventBus.html#send(java.lang.String,%20java.lang.Object)[send]`.
...
The object you receive in a message handler is a `link:../../scaladocs/io/vertx/scala/core/eventbus/Message.html[Message]`.

The `link:../../scaladocs/io/vertx/scala/core/eventbus/Message.html#body()[body]` of the message corresponds to the object that was sent or published.

The headers of the message are available with `link:../../scaladocs/io/vertx/scala/core/eventbus/Message.html#headers()[headers]`.

==== Acknowledging messages / sending replies

When using `link:../../scaladocs/io/vertx/scala/core/eventbus/EventBus.html#send(java.lang.String,%20java.lang.Object)[send]` the event bus attempts to deliver the message to a
`link:../../scaladocs/io/vertx/scala/core/eventbus/MessageConsumer.html[MessageConsumer]`registered with the event bus.

In some cases it's useful for the sender to know when the consumer has received the message and "processed" it.

To acknowledge that the message has been processed, the consumer can reply to the message by calling `link:../../scaladocs/io/vertx/scala/core/eventbus/Message.html#reply(java.lang.Object)[reply]`.
...
[source,scala]
----
eventBus.sendFuture("news.uk.sport", "Yay! Someone kicked a ball across a patch of grass").onComplete{
  case Success(result) => {
    println(s"Received reply: ${result.body()}")
  }
...
* The recipient has explicitly failed the message using `link:../../scaladocs/io/vertx/scala/core/eventbus/Message.html#fail(int,%20java.lang.String)[fail]`
...
Vertx.clusteredVertx(options, {
  case Success(result) => {
    ...
    println(s"We now have a clustered event bus: ${eventBus}")
  }
  case Failure(cause) => {
    println(s"$cause")
  }

...
The `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.EventBus.html[event bus]` is the *nervous system* of Vert.x.

There is a single event bus instance for every Vert.x instance and it is obtained using the method `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.Vertx.html#method_eventBus[eventBus]`.
...
Out of the box Vert.x allows any primitive/simple type, String, or `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.buffer.Buffer.html[buffers]` to
be sent as messages.
...
This simplest way to register a handler is using `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.EventBus.html#method_consumer[consumer]`.
Here's an example:
...
<?php
    ...
    $eb->consumer("news.uk.sport", function ($message) {
        echo "I have received a message: ".$message->body()."\n";
    });

----

When a message arrives for your handler, your handler will be called, passing in the `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.Message.html[message]`.

The object returned from call to +consumer()+ is an instance of `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.MessageConsumer.html[MessageConsumer]`.

This object can subsequently be used to unregister the handler, or use the handler as a stream.

Alternatively you can use `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.EventBus.html#method_consumer[consumer]` to
...
If you want to be notified when this has completed, you can register a `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.MessageConsumer.html#method_completionHandler[completion handler]`
...
<?php
    $consumer->completionHandler(function ($res, $res_err) {
        if ($res != null) {
            echo "The handler registration has reached all nodes\n";
        } else {
...
To unregister a handler, call `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.MessageConsumer.html#method_unregister[unregister]`.

If you are on a clustered event bus, un-registering can take some time to propagate across the nodes.  If you want to
be notified when this is complete, use `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.MessageConsumer.html#method_unregister[unregister]`.
...
<?php
    $consumer->unregister(function ($res, $res_err) {
        if ($res != null) {
            echo "The handler un-registration has reached all nodes\n";
        } else {
            echo "Un-registration failed!\n";
        };
    });
...
Publishing a message is simple. Just use `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.EventBus.html#method_publish[publish]` specifying the
...
You can send a message with `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.EventBus.html#method_send[send]`.
...
The object you receive in a message handler is a `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.Message.html[Message]`.

The `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.Message.html#method_body[body]` of the message corresponds to the object that was sent or published.

The headers of the message are available with `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.Message.html#method_headers[headers]`.

==== Acknowledging messages / sending replies

When using `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.EventBus.html#method_send[send]` the event bus attempts to deliver the message to a
`link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.MessageConsumer.html[MessageConsumer]`registered with the event bus.

In some cases it's useful for the sender to know when the consumer has received the message and "processed" it.

To acknowledge that the message has been processed, the consumer can reply to the message by calling `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.Message.html#method_reply[reply]`.
...
When sending a message with a reply handler, you can specify a timeout in the `link:../vertx-jphp/enums.adoc#DeliveryOptions[DeliveryOptions]`.
...
* The recipient has explicitly failed the message using `link:https://vertx.okou.tk/phpdoc/vertx-jphp/classes/io.vertx.jphp.core.eventbus.Message.html#method_fail[fail]`
...
<?php
    ...
    Vertx::clusteredVertx($options, function ($res, $res_err) {
        if ($res != null) {
            ...
            echo "We now have a clustered event bus: ".$eventBus."\n";
        } else {
            echo "Failed: ".$res_err."\n";
        };
    });
...
`link:../vertx-jphp/enums.adoc#EventBusOptions[EventBusOptions]`let you configure all aspects of these TCP connections. As
the event bus acts as a server and client, the configuration is close to
`link:../vertx-jphp/enums.adoc#NetClientOptions[NetClientOptions]`and `link:../vertx-jphp/enums.adoc#NetServerOptions[NetServerOptions]`.
...
The `link:../vertx-jphp/enums.adoc#EventBusOptions[EventBusOptions]` also lets you specify whether or not the event bus is
clustered, the port and host, as you would do with `link:../vertx-jphp/enums.adoc#VertxOptions#method_setClustered[setClustered]`,
`link:../vertx-jphp/enums.adoc#VertxOptions#method_getClusterHost[getClusterHost]` and `link:../vertx-jphp/enums.adoc#VertxOptions#method_getClusterPort[getClusterPort]`.

...
[source,jphp]
----
/*variableDecl*/$eb = $vertx.$eventBus()
...
/*variableDecl*/$consumer = $eb.$consumer<Any>("news.uk.sport")
...
[source,jphp]
----
/*variableDecl*/$consumer = $eventBus.$consumer<Any>("news.uk.sport")
$consumer.$handler({ message ->
  echo "I have received a message: ${$message.$body()}"."\n";
...
$eventBus.$send<Any>("news.uk.sport", "Yay! Someone kicked a ball across a patch of grass", { ar ->
  if (ar.succeeded()) {
    echo "Received reply: ${ar.result().$body()}"."\n";
  }
})
...
[source,jphp]
----
/*variableDecl*/$options = VertxOptions()
Vertx.$clusteredVertx($options, { res ->
  if (res.succeeded()) {
    /*variableDecl*/$vertx = res.result()
    /*variableDecl*/$eventBus = $vertx.$eventBus()
    echo "We now have a clustered event bus: ${$eventBus}"."\n";
  } else {
    echo "Failed: ${res.cause()}"."\n";
  }
})

...
public interface TestApi<TV, TTT> /*extends Measured, WriteStream<Object>*/{
    ...
    //basic
    String testString(@Nullable String str);
    byte testByte(byte i);
    Byte testByteBoxed(Byte i);
    char testChar(char i);
    Character testCharacter(Character i);
    Short testShortBoxed(Short i);
    short testShort(short i);
    Integer testIntegerBoxed(Integer i);
    int testInteger(int i);
    long testLong(long i);
    Long testLongBoxed(Long i);
    Double testDoubleBoxed(Double i);
    double testDouble(double i);
    Float testFloatBoxed(Float i);
    float testFloat(float i);
    Boolean testBooleanBoxed(Boolean i);
    boolean testBoolean(boolean i);

    //json
    JsonObject testJsonObject(JsonObject json);
    JsonArray testJsonArray(JsonArray json);

    //dataobject
    HttpServerOptions testDataObject(HttpServerOptions options);

    //enum
    HttpVersion testEnum(HttpVersion v);

    //throwable
    Throwable testThrowable(Throwable t);

    //V
    <V> V testVariableV(V o);//
    TV testVariableTV(TV o);//

    //Object
    Object testObject(Object o);//

    //VertxGen<T>,T is not null, VertxGen, <X>, VOID, basic, json, DaaObject, Enum

    //VertxGen<T>
    TestApi<TV, TTT> testVertxGenV1(TestApi<TV, TTT> f);
    //VertxGen<X>
    <V>Future<V> testVertxGenV2(Future<V> f);
    //VertxGen<VertxGen>
    ...
    void testHandler(Handler<Future<Boolean>> h);
